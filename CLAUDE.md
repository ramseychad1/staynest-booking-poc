...
`staging` also has a Railway Bucket (`staynest-uploads`) wired into the backend service via variable references (`BUCKET`/`ACCESS_KEY_ID`/`SECRET_ACCESS_KEY`/`ENDPOINT`/`REGION` = `${{staynest-uploads.VARNAME}}`) — property/blog image uploads go there, not local disk. See the storage/media notes under the backend architecture section below for why direct bucket URLs don't work and what's proxying them.
...
- **The admin panel's `vite.config.js` CSP and the frontend's `next.config.mjs` `images.remotePatterns` both need the backend's actual domain, or the browser silently blocks the request/image with zero server-side trace.** Both were originally hardcoded to `api.thekeysvibe.com` / localhost only; requests and images to the real deployed backend were blocked client-side before ever leaving the page — the backend's own access logs showed nothing wrong because the request never arrived. Confirmed via real browser DevTools (CSP violations show in the Console, blocked requests show `(blocked:csp)` in the Network tab) — this is invisible to `curl`, which doesn't enforce CSP or do CORS preflights, so a working `curl` test does **not** mean the browser will work. Both configs now derive the allowlisted origin from `VITE_API_URL` / `NEXT_PUBLIC_API_BASE_URL` at config-load time instead of a hardcoded domain, so this shouldn't need editing again per environment — but if you add a *new* external origin the app talks to (a different API, a CDN, etc.), it needs to go in both places by hand.
